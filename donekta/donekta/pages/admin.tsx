...
import { CheckCircle, XCircle, Clock, Heart, Copy, Check, Trash2, Users, DollarSign, TrendingUp } from 'lucide-react'
        ...
        <div className="max-w-5xl mx-auto px-6 py-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <p className="text-2xl font-black text-gray-900">{donations.length}</p>
              <p className="text-sm text-gray-500">Donaciones recientes</p>
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
                ...
                {donations.map((d: any) => (
                  <div key={d.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                    <div>
                      ...
                      <p className="text-xs text-gray-400">→ {d.communities?.name || 'Comunidad'} · {new Date(d.created_at).toLocaleDateString('es-MX', { day: 'numeric', month: 'short' })}</p>
                    </div>
          ...
          {/* Communities */}
          <h2 className="text-xl font-black text-gray-900 mb-4">Solicitudes de comunidades</h2>
                ...
                <div key={c.id} className="bg-white rounded-2xl border border-gray-100 p-6">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      {c.image_url && (
                        <img src={c.image_url} alt={c.name} className="w-16 h-16 rounded-xl object-cover mb-3" />
                      )}
                      ...
                      {c.category && <p className="text-xs text-gray-400 mb-1">🏷️ {c.category}</p>}
                      {c.city && <p className="text-xs text-gray-400 mb-1">📍 {c.city}{c.state ? `, ${c.state}` : ''}</p>}
                      {c.mission && <p className="text-xs text-gray-500 mb-2 line-clamp-2">{c.mission}</p>}
                      ...
                      {approvingId === c.id && (
                        <div className="mt-4 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
                          ...
                          <p className="text-xs text-emerald-700 mb-3">La usará para editar su perfil</p>
                          ...
                          <div className="flex gap-2">
                            ...
                            <button onClick={() => setApprovingId(null)}
                              className="border border-gray-200 text-gray-600 text-xs font-semibold px-4 py-2 rounded-lg hover:bg-gray-50">
                              Cancelar
                            </button>
                          </div>
                        </div>
